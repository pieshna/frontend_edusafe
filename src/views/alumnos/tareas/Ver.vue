      ...
      <div class="table table-hover">
        ...
        <tbody>
          <tr v-for="tarea in tareasDisponibles" :key="tarea.id">
            ...
            <td>
              <div v-for="nota in notasRecibidas" :key="nota.id">
                <p v-if="nota.tareaId == tarea.id">{{ nota.punteo }}</p>
              </div>
            </td>
            <td>{{ tarea.punteo }} Pts.</td>
            <td>
              <router-link :to="'/nueva/tarea/' + tarea.id">
                <a>
                  <svg
...
export default {
  ...
  methods: {
    async obtenerTareas() {
      ...
      await axios.get(this.host + "vernota/" + this.$localStorage.get("teken")).then((res) => {
        this.notasRecibidas = res.data;
      });
    },
  },
  created() {
